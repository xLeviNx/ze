...
#define RANDOM_1		   VIP_J
#define RANDOM_2		   VIP_K
#define RANDOM_3		   VIP_L	
#define VIP_USER           VIP_Z
...
ReadFile()
{
	...
	if(iFilePointer)
	{
		...
		if(g_bRankSystem)
		{
			iMaxLevels = get_cvar_num("ze_levels_maximum")
		}
...
public Cmd_Knife(id)
{
	...
	for(new iFlags = ze_get_vip_flags(id), i; i < g_iKnivesNum; i++)
	{
		ArrayGetArray(g_aKnives, i, eKnife)
		...
		if(g_bRankSystem)
		{
			if(eKnife[LEVEL] && iLevel < eKnife[LEVEL])
			{
				...
				{
					static szRank[32]
					ze_get_user_level(eKnife[LEVEL])
...
{
	if(is_user_alive(id) && g_eCvars[km_open_at_spawn] && !g_iKnife[id] && g_bFirstTime[id] && (g_iMenuFlags & ADMIN_USER || ze_get_vip_flags(id) & g_iMenuFlags))
	{
...
bool:has_knife_access(const id, const iKnife)
{
	...
	{
		if(ze_get_vip_flags(id) & eKnife[FLAG])
		{
